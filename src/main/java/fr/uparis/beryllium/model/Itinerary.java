 ...
 * and therefore there is no need to specify these stations as attributes of
 * this class
 */
public class Itinerary {
	...
	/**
	 * For each station we pass through, we store the time at which we boarded the
	 * train at that station here
	 */
	 ...
	 * best duration of travel for each station (compared to our fixed starting
	 * station)
	 */
	...
	private void init(Station start) {
		for (Station s : allStations) {
			MutableTriple<Double, Integer, Long> distCountTime = new MutableTriple<>(Double.MAX_VALUE,
	...
	private Station bestStationByPreference(ArrayList<Station> notVisited, int preference) {
		...
		for (Station s : notVisited) {
			...
			switch (preference) {
				...
				default -> throw new IllegalArgumentException("Invalid preference value " + preference
	...
	private void updateDistCountTime(Station station, Integer preference, LocalTime actualTime) {
		HashMap<Station, ArrayList<NeighborData>> neighborsOfStation = station.getNextStations();
		MutableTriple<Double, Integer, Long> stationDistCountTime = distCountTimeToStart.get(station);
		...
		neighborsOfStation.forEach((neighborStation, nDataArray) -> {
			for (NeighborData nData : nDataArray) {
				...
				if (neighborDistCountTime != null) {
					switch (preference) {
						case 0 -> {
							if ((Long.compare(timeToWait, Long.MAX_VALUE) != 0)
									&&
						...
						case 1 -> {
							if (Long.compare(timeWeight, neighborDistCountTime.getRight()) < 0)
								swap = true;
						}
						case 2 -> {
							if ((Long.compare(timeToWait, Long.MAX_VALUE) != 0)
						...
						default -> {
							throw new IllegalArgumentException("Invalid preference value " + preference
									+ ". It's supposed to be a value between 0 and 2");
						}
					}
					if (swap) {
	...
	/**
	 * Computes the shortest path between all pairs of start and dest stations
	 * defined in
	 * the corresponding arraylists using the shortestWay algorithm on each pair:
	 * For each station in <code>start</code> and each station in <code>dest</code>,
	 ...
	 * @param preference 0: shortest distance, 1: best time, 2: closest in the tree
	 * @return the best path possible between a station called X and a station
	 *         called Y
	 * @see Station
	 * @throws IllegalArgumentException when you choose a preference other than 0, 1
	 *                                  or 2
	 */
	public HashMap<Station, Line> shortestMultiplePaths(ArrayList<Station> start, ArrayList<Station> dest,
			Integer preference, LocalTime timeWeLeft) {
	...
	/**
	 * This method is used to return an ordered list of all the lines in a path
	 *
	 * @param res HashMap of the path calculated by shortestWay Method
	 * @return An ordered list of lines in the path (can be empty)
	 */
	...
	/**
	 * This method is used to return an ordered list of all the times for each
	 * station in a path
	 *
	 * @param res HashMap of the path calculated by shortestWay Method
	 * @return An ordered list of the times in the path (can be empty)
	 */
	...
	/**
	 * This method is used to return an ordered list of all distances, amount of
	 * stations and best times
	 * for each station in a path
	 *
	 * @param res HashMap of the path calculated by shortestWay Method
	 * @return An ordered list of all distances, amount of stations and best times
	 *         in the path (can be empty)
	 */
	public LinkedList<MutableTriple<Double, Integer, Long>> getDistCountTimes(HashMap<Station, Line> res) {
		...
		for (Station s : res.keySet()) {
			...
			MutableTriple<Double, Integer, Long> newTime = new MutableTriple<>(tmp.getLeft(), tmp.getMiddle(),
					d.toMinutes() + 1);
	 ...
	 * @return a couple of distance/time which represent the distance and time to
	 *         travel the line of the station at the position
	 */
	public MutablePair<Double, Long> getDistTimeForALine(ArrayList<Station> stationRes, ArrayList<Line> lineRes,
			int position) {
		...
		while (tempPos < stationRes.size()) {
			if (lineRes.get(position + 1) != lineRes.get(tempPos))
				break;
			tempPos++;
		}
		MutableTriple<Double, Integer, Long> distTimeDestination = this.distCountTimeToStart
				.get(stationRes.get(tempPos - 1));
